...
class AckermannVision:
    ...
    def process_frame(self):
        ...
        # --- Black lane detection (original mask for center line detection) ---
        ...
        for cnt in white_contours:
            if cv2.contourArea(cnt) > 50:
                cv2.drawContours(roi, [cnt], -1, (255, 0, 0), 2)  # Blue boundary for white lane
        ...
        for cnt in yellow_contours:
            if cv2.contourArea(cnt) > 50:
                cv2.drawContours(roi, [cnt], -1, (0, 140, 255), 2)  # Orange boundary for yellow lane
    ...
    def draw_trajectory(self, roi, steering_angle_deg):
        ...
        steering_angle_rad = np.radians(steering_angle_deg - 75)
        ...
        for y in range(0, roi.shape[0], 5):
            dy = y
            if abs(radius_px) > 1e-2 and dy < abs(radius_px):
            ...
            cv2.circle(roi, (pt_x_center, pt_y), 1, (0, 255, 255), -1)

            pt_x_left = int(pt_x_center - (track_width_px / 2))
            cv2.circle(roi, (pt_x_left, pt_y), 1, (0, 255, 0), -1)

            pt_x_right = int(pt_x_center + (track_width_px / 2))
            cv2.circle(roi, (pt_x_right, pt_y), 1, (0, 0, 255), -1)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if roi is None or mask is None:
            print("No ROI or mask, skipping frame...")
            continue
        ...
        cv2.imshow("ROI Mask", cv2.resize(mask, (320, 80)))  # resize mask for better display
